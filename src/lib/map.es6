...
var TileMaps = window.TileMaps;
if (typeof(TileMaps) != "undefined")
	for (const i in TileMaps)
		preloadImages(TileMaps[i]);

window.onTileMapLoaded = (name, data) => {
	preloadImages(data);
	TileMaps[name] = data;
};
...
function TiledSet(data) {
	...
	const w = data.tilewidth;
	const h = data.tileheight;
	const l = data.imagewidth / data.tilewidth;
