	...
	getImagePaths() {
		...
		for (const map in this.data) {
			...
			for (let i = 0; i < this.data[map].layers.length; i++) {
				if (this.data[map].layers[i].type == "imagelayer") {
					let imagepath = this.data[map].layers[i].image;
					if (mappath.length) imagepath = mappath + '/' + imagepath;
					images.push(imagepath);
					this.data[map].layers[i].image = imagepath;
				}
			}
		}
...
export class TiledMap extends Entity {
	constructor(data, pos) {
		...
		for (let i = 0; i < this.data.layers.length; i++) {
			let layer;
			switch (this.data.layers[i].type) {
				case  "tilelayer":
					layer = new TiledLayer(this.data.layers[i], this);
					break;
				case "imagelayer":
					layer = new TiledImageLayer(this.data.layers[i], this);
			}
			if (layer) {
				this.add(layer);
