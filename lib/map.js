...
class TiledTileset {
	...
	draw(ctx, gid, x, y, transforms, shit) {
		...
		ctx.drawImage(this.img,
			...
			drawwidth * scalex,
			drawheight * scaley);
		if (shit) console.log("drawing: " + x + ", " + y+ ", " + drawwidth + ", " + drawheight);
		ctx.restore();
	}
...
class TiledPalette {
	...
	draw(ctx, gid, x, y, shit) {
		...
		if (shit) { console.log(gid); console.log(transforms); }

		for (const tileset of this.sets) {
			if (tileset.contains(gid))
				tileset.draw(ctx, gid, x, y, transforms, shit);
		}
	}
	...
	render(ctx) {
		...
		for (const tile of this.tiles) {
			if (tile.data.visible) {
				console.log(tile);
				this.parent.palette.draw(ctx, tile.data.gid, tile.data.x, tile.data.y, true);
			}
		}
	}
