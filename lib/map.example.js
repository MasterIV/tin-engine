...
class VariousExampleScene extends Scene {
	constructor() {
		...
		this.loader = new TiledDataLoader();
		// Preloading.
		this.loader.preloadCompleteJSON(['examples/map2.json', 'examples/map3.json'], () => {
			// New hexagonal map.
			const map = new TiledMap(this.loader.data['examples/map2.json'], new V2(50,50));
			// New isometric map.
			const map2 = new TiledMap(this.loader.data['examples/map3.json'], new V2(130,150));
			// Add the maps for live drawing.
			this.add(map);
			this.add(map2);
		});
	}
...
export default [
	{
		...
		description: `In this basic example, a simple Tiled map will be displayed.
			...
			On the left, you can see the map drawn.
			...
			These can be loaded as regular javascripts through &lsaquo;script&rsaquo; tags in your base HTML file.
			Using this method, you don't have to call specific file loading function except for image preloading.<br><br>
			...
			'callback' can be a function that will be called once all files have been loaded.<br><br>
			<b>async loadTilesets()</b> - Same as preloadTilesets but asynchronous.<br><br>
			...
			<b>preloadCompleteJSON(paths, callback)</b> - This method will do all the steps above in order.
			...
			In the above example, the map entity is instructed to draw the map once onto a hidden canvas element with 'staticRender'.
			...
			<b>staticRender(merge, filter)</b> - Will draw the map on a separate, hidden canvas and only this image will be drawn into the game canvas each frame.
	...
	},
	{
		...
		The importer <b>does not</b> support: tintcolor, regardless of where it is used; compression of layer data (gzip, zlib, ...) - base64 encoding is supported though; parallaxoriginx/y and parallaxx/parallaxy; infinite maps; single tiles in object layers.<br>
		If you wish to access any values of the map, the full JSON will be saved in the "data" property of the map entity.<br>
		<br>
